  ...
  return (
    <main className="min-h-screen bg-cream text-stone-900">
      <div className="mx-auto max-w-3xl px-6 py-14">
        ...
        <h1 className="mt-4 font-display text-4xl font-extrabold">Family dashboard</h1>
        ...
        <h2 className="mt-10 font-display text-2xl font-bold">Receipts</h2>
        {receipts.length === 0 && (
        ...
        <ul className="mt-3 space-y-3">
          {receipts.map((r) => (
            <li key={r.receiptId} className="rounded-2xl bg-white p-5 shadow-sm">
              ...
              <Link href={`/verify/${r.receiptId}`} className="mt-2 inline-block text-sm font-medium font-bold text-crimson underline">
                Verify receipt →
              </Link>
            </li>
          ))}
        ...
        <h2 className="mt-10 font-display text-2xl font-bold">Visit calendar</h2>
        <ul className="mt-3 space-y-2">
          {slots.map((s) => (
            <li key={s.id} className="flex items-center justify-between rounded-xl bg-white px-4 py-3 shadow-sm">
              ...
              <span className={`rounded-full px-3 py-1 text-xs font-semibold ${s.available ? "bg-blush text-crimson-deep" : "bg-stone-200 text-stone-600"}`}>
